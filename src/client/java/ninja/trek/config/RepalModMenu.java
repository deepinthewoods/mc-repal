...
public class RepalModMenu implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        // Instead of returning a Cloth Config screen, we return our custom screen.
        return parent -> new RepalConfigScreen(parent);
    }

    // --- Merged RepalConfigScreen (custom config screen) below ---
    public static class RepalConfigScreen extends Screen {
        ...
        protected void init() {
            int centerX = width / 2;
            int startY = height / 4;
            // Initialize any widgets here if needed...
        ...
        private void loadPreviewTexture() {
            try {
                // Load the original grass texture from Minecraft's resources
        ...
        private void updatePreview() {
            ...
            try {
                // Process the preview image using the current palette and adjustments
                ...
                // Convert the processed BufferedImage to a NativeImage
                ByteArrayOutputStream os = new ByteArrayOutputStream();
                ImageIO.write(processed, "png", os);
                ByteArrayInputStream is = new ByteArrayInputStream(os.toByteArray());
                NativeImage nativeImage = NativeImage.read(is);
                // Update the texture used for previewing
        ...
        public void render(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            // Draw the original texture preview (left side)
            context.drawTexture(
                    GRASS_PREVIEW,         // Texture identifier
                    width / 4 - 32,        // x position
                    height / 2,            // y position
                    0,                     // z (depth)
            ...
            // Draw the processed texture preview (right side)
            context.drawTexture(
                    RepalClient.PREVIEW_TEXTURE_ID, // Texture identifier for the processed image
                    3 * width / 4 - 32,               // x position
                    height / 2,                       // y position
                    0,                                // z (depth)
                    0.0f,                             // u coordinate start
                    0.0f,                             // v coordinate start
            ...
            // Draw labels above each preview
            ...
            // Render any additional widgets or buttons
